...
export default function NewDealPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!form.title.trim() || !form.value || !form.contactId) {
  ...
  return (
    <div className="max-w-2xl">
      <div className="mb-8">
        <Link href="/deals" className="text-sm text-slate-400 hover:text-blue-400 flex items-center gap-1 mb-4">
      ...
      <form onSubmit={handleSubmit} className="bg-[#1e293b] rounded-xl border border-[#334155] p-6 space-y-5">
        <div>
          <label className="block text-sm font-medium text-slate-300 mb-1.5">Deal Title *</label>
          <input
            type="text"
            value={form.title}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div>
            ...
            <select
              ...
              className="w-full bg-[#0f172a] border border-[#334155] rounded-lg px-4 py-2.5 text-sm text-white focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
            >
              {STAGES.map((s) => (
        ...
        <div className="flex items-center gap-3 pt-2">
          ...
          <Link href="/deals" className="text-slate-400 hover:text-slate-300 px-4 py-2.5 text-sm transition-colors">
            Cancel
          </Link>
        </div>
      </form>
