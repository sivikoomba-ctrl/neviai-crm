...
export async function smartSearch(query: string): Promise<SearchResult[]> {
  ...
  const contactSummaries = contacts.map(
    (c) =>
      `CONTACT|${c.id}|${c.name}|${c.email}|${c.company || ""}|${c.tags || ""}|deals:${c.deals.length}|interactions:${c.interactions.length}`
  );

  const dealSummaries = deals.map(
    (d) =>
      `DEAL|${d.id}|${d.title}|$${d.value}|${d.stage}|contact:${d.contact.name}`
  );
