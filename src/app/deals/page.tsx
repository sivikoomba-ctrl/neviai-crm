...
export default function DealsPage() {
  ...
  function stageValue(stage: string) {
    return dealsByStage(stage).reduce((s, d) => s + d.value, 0);
  }
    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-slate-400 mt-1">{deals.length} total deals &middot; ${deals.reduce((s, d) => s + d.value, 0).toLocaleString()} pipeline value</p>
        </div>
        <Link
          href="/deals/new"
            ...
            <div key={stage} className={`flex-shrink-0 w-72 rounded-xl border ${colors.border} ${colors.bg}`}>
              <div className={`${colors.header} rounded-t-xl px-4 py-3 flex items-center justify-between`}>
                <div className="flex items-center gap-2">
                  ...
                  <span className="bg-white/20 text-white text-xs px-2 py-0.5 rounded-full">{stageDeals.length}</span>
                </div>
              ...
              <div className="p-3 space-y-3 min-h-[200px] max-h-[calc(100vh-300px)] overflow-y-auto">
                {stageDeals.length === 0 ? (
                  <p className="text-slate-500 text-xs text-center py-8">No deals</p>
                ) : (
                  stageDeals.map((deal) => (
                    <Link
                      key={deal.id}
                      href={`/deals/${deal.id}`}
                      className="block bg-[#1e293b] rounded-lg p-4 border border-[#334155] hover:border-[#475569] transition-colors group"
                    >
                      ...
                      <div className="flex items-center justify-between mt-3">
                        <span className="text-sm font-semibold text-green-400">${deal.value.toLocaleString()}</span>
                        {deal.aiScore !== null && (
                          <span className={`text-xs px-2 py-0.5 rounded-full ${
                            deal.aiScore >= 75 ? "bg-green-500/20 text-green-300" :
                            deal.aiScore >= 50 ? "bg-yellow-500/20 text-yellow-300" :
