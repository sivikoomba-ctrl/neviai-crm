...
export default function ContactDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function loadContact() {
    try {
      ...
      setContact(data);
      const tags = data.tags ? JSON.parse(data.tags).join(", ") : "";
      setEditForm({
        name: data.name,
        email: data.email,
        phone: data.phone || "",
  ...
  async function handleEdit(e: React.FormEvent) {
    ...
    try {
      ...
      if (editForm.tags.trim()) {
        tagsJson = JSON.stringify(editForm.tags.split(",").map((t) => t.trim()).filter(Boolean));
      }
      const res = await fetch(`/api/contacts/${id}`, {
        method: "PUT",
  ...
  async function generateSummary() {
    ...
    try {
      const res = await fetch(`/api/contacts/${id}/summary`, { method: "POST" });
  ...
  async function addInteraction(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) {
        setInteractionForm({ type: "email", subject: "", body: "" });
        setShowInteractionForm(false);
        loadContact();
      }
    } catch (err) {
      console.error("Interaction creation failed:", err);
    }
  }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
              ...
              <form onSubmit={handleEdit} className="space-y-4">
                ...
                <div>
                  ...
                  <textarea value={editForm.notes} onChange={(e) => setEditForm({ ...editForm, notes: e.target.value })} rows={3}
                    className="w-full bg-[#0f172a] border border-[#334155] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-blue-500 resize-none" />
                </div>
                <div className="flex gap-2">
                  ...
                  <div className="col-span-2">
                    ...
                    <div className="flex flex-wrap gap-1 mt-1">
                      {parseTags(contact.tags).map((tag) => (
                        <span key={tag} className="bg-blue-500/20 text-blue-300 text-xs px-2 py-0.5 rounded-full">{tag}</span>
                      ))}
                    </div>
                  </div>
                )}
                {contact.notes && (
                  <div className="col-span-2"><span className="text-slate-400">Notes:</span><p className="text-white mt-1">{contact.notes}</p></div>
                )}
              </div>
            )}
          </div>
          ...
          <div className="bg-[#1e293b] rounded-xl border border-[#334155]">
            ...
            <div className="p-6">
              {contact.deals.length === 0 ? (
                <p className="text-slate-400 text-sm">No deals associated.</p>
              ) : (
                  ...
                  {contact.deals.map((deal) => (
                    <Link key={deal.id} href={`/deals/${deal.id}`} className="flex items-center justify-between p-3 rounded-lg hover:bg-[#334155] transition-colors">
                      ...
                      <div className="text-right">
                        <p className="text-sm font-semibold text-green-400">${deal.value.toLocaleString()}</p>
                        <span className={`text-xs text-white px-2 py-0.5 rounded-full ${stageColors[deal.stage] || "bg-slate-500"}`}>{deal.stage}</span>
                      </div>
                    </Link>
                  ))}
                </div>
          ...
          <div className="bg-[#1e293b] rounded-xl border border-[#334155]">
            <div className="p-6 border-b border-[#334155] flex items-center justify-between">
              ...
              <button onClick={() => setShowInteractionForm(!showInteractionForm)} className="text-sm text-blue-400 hover:text-blue-300">
              ...
              {showInteractionForm && (
                <form onSubmit={addInteraction} className="mb-6 space-y-3 p-4 bg-[#0f172a] rounded-lg border border-[#334155]">
                  ...
                  <div>
                    ...
                    <textarea value={interactionForm.body} onChange={(e) => setInteractionForm({ ...interactionForm, body: e.target.value })} rows={3}
                      className="w-full bg-[#1e293b] border border-[#334155] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-blue-500 resize-none"
                ...
                <div className="space-y-3">
                  {contact.interactions.map((interaction) => (
                    <div key={interaction.id} className="flex items-start gap-3 p-3 rounded-lg hover:bg-[#334155]/50 transition-colors">
                      <div className="p-2 bg-[#0f172a] rounded-lg shrink-0">
                        <svg className="w-4 h-4 text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={typeIcons[interaction.type] || typeIcons.note} />
                        </svg>
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-medium text-blue-400 uppercase">{interaction.type}</span>
                          <span className="text-xs text-slate-500">{new Date(interaction.createdAt).toLocaleString()}</span>
                        </div>
        ...
        <div className="space-y-6">
          <div className="bg-[#1e293b] rounded-xl border border-[#334155] p-6">
            ...
            {contact.aiSummary ? (
              <p className="text-sm text-slate-300 leading-relaxed">{contact.aiSummary}</p>
            ) : (
            ...
            <button
              ...
              className="mt-4 w-full bg-blue-600/20 hover:bg-blue-600/30 text-blue-400 px-4 py-2 rounded-lg text-sm font-medium transition-colors disabled:opacity-50 border border-blue-500/30"
            >
          ...
          <div className="bg-[#1e293b] rounded-xl border border-[#334155] p-6">
            ...
            <div className="space-y-3">
              <div className="flex justify-between text-sm">
                ...
                <span className="text-white font-medium">{contact.deals.length}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-slate-400">Pipeline Value</span>
                <span className="text-green-400 font-medium">${contact.deals.reduce((s, d) => s + d.value, 0).toLocaleString()}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-slate-400">Interactions</span>
                <span className="text-white font-medium">{contact.interactions.length}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-slate-400">Won Deals</span>
                <span className="text-white font-medium">{contact.deals.filter((d) => d.stage === "Won").length}</span>
              </div>
            </div>
          </div>
        </div>
