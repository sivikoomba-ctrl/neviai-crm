...
export default function DealDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function loadDeal() {
    try {
      ...
      setDeal(data);
      setEditForm({ title: data.title, value: String(data.value), stage: data.stage });
    } catch {
      router.push("/deals");
    } finally {
      setLoading(false);
  ...
  async function handleEdit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) { setEditing(false); loadDeal(); }
    } catch (err) {
  ...
  async function scoreDeal() {
    ...
    try {
      const res = await fetch(`/api/deals/${id}/score`, { method: "POST" });
      if (res.ok) loadDeal();
    } catch (err) {
  ...
  const scoreColor = deal.aiScore !== null
    ? deal.aiScore >= 75 ? "text-green-400" : deal.aiScore >= 50 ? "text-yellow-400" : "text-red-400"
    : "text-slate-400";
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="flex items-start justify-between mb-6">
              <div>
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className={`text-xs text-white px-3 py-1 rounded-full ${stageColors[deal.stage] || "bg-slate-500"}`}>{deal.stage}</span>
                </div>
                <p className="text-2xl font-bold text-green-400">${deal.value.toLocaleString()}</p>
              </div>
          ...
          <div className="bg-[#1e293b] rounded-xl border border-[#334155] p-6">
            ...
            <Link href={`/contacts/${deal.contact.id}`} className="flex items-center gap-4 p-4 rounded-lg hover:bg-[#334155] transition-colors border border-[#334155]">
              ...
              <div>
                <p className="font-medium text-white">{deal.contact.name}</p>
                <p className="text-sm text-slate-400">{deal.contact.email}</p>
                {deal.contact.company && <p className="text-xs text-slate-500">{deal.contact.company}</p>}
              </div>
            </Link>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="bg-[#1e293b] rounded-xl border border-[#334155] p-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-sm font-semibold text-white">AI Deal Score</h3>
            </div>
            {deal.aiScore !== null ? (
              <div>
                <div className="flex items-center gap-4 mb-4">
                  <div className={`text-4xl font-bold ${scoreColor}`}>{deal.aiScore}</div>
                  <div className="text-sm text-slate-400">/100</div>
                </div>
                <div className="w-full bg-[#0f172a] rounded-full h-2 mb-4">
                  <div
                    className={`h-2 rounded-full transition-all ${
                      deal.aiScore >= 75 ? "bg-green-500" : deal.aiScore >= 50 ? "bg-yellow-500" : "bg-red-500"
                    }`}
                ...
                {deal.aiAdvice && (
                  <div className="bg-[#0f172a] rounded-lg p-3 border border-[#334155]">
                    <p className="text-xs text-slate-400 mb-1">AI Advice</p>
                    <p className="text-sm text-slate-300">{deal.aiAdvice}</p>
                  </div>
                )}
              </div>
            ) : (
            ...
            >
              {scoreLoading ? "Scoring..." : deal.aiScore !== null ? "Re-score Deal" : "Score Deal"}
            </button>
          </div>
              ...
              {STAGES.filter((s) => s !== "Lost").map((stage) => {
                ...
                return (
                  <div key={stage} className="flex items-center gap-3">
                    <div className={`w-3 h-3 rounded-full ${isActive ? "bg-blue-500 ring-2 ring-blue-500/30" : isPast ? "bg-green-500" : "bg-[#334155]"}`} />
                    <span className={`text-sm ${isActive ? "text-white font-medium" : isPast ? "text-green-400" : "text-slate-500"}`}>{stage}</span>
                  </div>
                );
              })}
            </div>
